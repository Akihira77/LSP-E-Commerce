...
import { getInvoiceDetails } from "../utils/api.ts";
...
export default function InvoicePage() {
	...
	React.useEffect(() => {
		const getTransactionDetails = async () => {
			const { data } = await getInvoiceDetails(Number(id));
			console.log(data);
		...
		<div className="flex h-screen w-full items-center justify-center bg-gray-600">
			<div className="w-80 rounded bg-gray-50 px-6 pt-8 shadow-lg">
				...
				<div className="flex flex-col justify-center items-center gap-2">
					<h4 className="font-semibold">E-Commerce LSP</h4>
					<p className="text-xs">Poncowati, Lampung Tengah</p>
				</div>
				<div className="flex flex-col gap-3 border-b py-6 text-xs">
					...
					<p className="flex justify-between">
						...
						<span>Andika Wahyu Permadi</span>
					</p>
					<p className="flex justify-between">
				...
				<div className="flex flex-col gap-3 pb-3 pt-2 text-xs">
					<table className="w-full text-left">
						...
						<tbody>
							{transactionDetails &&
								transactionDetails.transactions.length > 0 &&
								transactionDetails.transactions.map((item) => (
				...
				<div className="flex justify-center mb-3">
					<a
						...
						className="text-sky-500 border-b border-sky-700"
					>
						Back to Homepage
					</a>
				</div>
